...
if st.button("Generate PDF Report"):
    ...
    pdf.cell(200, 10, txt = "Price Data:", ln = True)
    pdf.ln(5)
    
    pdf.set_font("Arial", size = 10)
    for index, row in df.iterrows():
        pdf.cell(200, 10, txt = f"{row['Arrival_Date'].strftime('%Y-%m-%d')} : ₹{row['Modal Price']}", ln = True)
    ...
    st.download_button(
        ...
        data=pdf_output,
        file_name='commodity_price_report.pdf',
