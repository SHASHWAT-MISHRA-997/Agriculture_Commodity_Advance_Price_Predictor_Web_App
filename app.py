...
def download_report(state, district, market, commodity, predicted_price, selected_date):
    ...
    def add_image_to_pdf(image, title):
        if image:
            ...
            pdf.image(image, x=10, y=pdf.get_y() + 10, w=190)
            pdf.ln(10)
        else:
    ...
    price_trend_img = plot_prices()  # Ensure this function returns a file path
    add_image_to_pdf(price_trend_img, "Price Trend")

    correlation_heatmap_img = show_correlation_heatmap()  # Ensure this function returns a file path
    add_image_to_pdf(correlation_heatmap_img, "Correlation Heatmap")

    price_comparison_img = price_comparison()  # Ensure this function returns a file path
    add_image_to_pdf(price_comparison_img, "Price Comparison")

    price_volatility_img = plot_price_volatility()  # Ensure this function returns a file path
